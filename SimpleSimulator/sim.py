...
def mov_f(a):#(a, b):  # changed
    # registers[a] = b
    global V,L,G,E
    bin_val=V+L+G+E
    registers[a] = int(bin_val,2)
    registers_display[a] = BFiller(registers[a])
...
def div(b, c):
    if(registers[c]==0):
        print("ERROR @Cycle No. "+str(c_count)+": Divison by zero error")
        quit()
    if(registers[c]!=0):
...
def fun_for_sim(p_count):
    ...
    instruction_root = memory_dis[p_count]#instruction_root_index]
    # if instruction_root == "\n":
    #     continue
    ...
    if interpreted == "A":
        ...
        # freset()
        if opcode == '00011' and b in range(7):
            mov(a, b)
        elif opcode == '00011' and b==7:
            mov_f(a)
        if opcode == '00111':
            div(a, b)
        if opcode == '01101':
        ...
        if opcode == "01111":
            ...
            #return
        if opcode == "10000":
            p_count=jlt(a,p_count)
            # return
        if opcode == "10001":
            p_count=jgt(a,p_count)
            #jgt(a)
            #return
        if opcode == "10010":
            p_count=je(a,p_count)
            return
        freset()
...
y_axis = []
def main():
    global pc,registers_display,V,L,G,E,memory_dis,x_axis,y_axis
    p_count = 0
    c_count = 0
    while(True):
        fh=0
        x_axis.append(c_count)
        y_axis.append(p_count)
        o= fun_for_sim(p_count)
        np_count=o[0]
        fh=o[1]
        print(AddressFiller(p_count), end=" ")
        p_count = np_count
        for i in registers_display:
            print(i, end=" ")
        print(('0'*12)+V+L+G+E)
        c_count+= 1
        if(fh==1):
