...
try:
    ...
    for preprocessor in code.splitlines(): #for preprocessor in code.readlines():  #changed
        ...
        if preprocessor == "\n" or preprocessor == '':
            continue
        if (hlt_c>0):
            print("ERROR @Line "+str(linec)+": Misplaced halt error(hlt is not the last instruction and is followed by other code line/s. Also, hlt should only be declared once)")
            quit()
