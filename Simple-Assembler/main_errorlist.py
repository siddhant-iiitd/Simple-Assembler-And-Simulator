...
try:
    ...
    for preprocessor in code.splitlines():  # for preprocessor in code.readlines():  #changed
        ...
        if preprocessor == "\n":
            continue
        if (hlt_c > 0):
            errors += "ERROR @Line "+str(linec)+": Misplaced halt error(hlt is not the last instruction and is followed by other code line/s. Also, hlt should only be declared once)" + "\n"
           # quit()
        ...
        if instruct_root_p[0].lower() == "var":
            if vflag == 1:
                errors += "ERROR @Line "+str(linec) + ": Declare variables only at the top" + "\n"
                #quit()
            if len(instruct_root_p) != 2:
                errors += "ERROR @Line "+str(linec) + ": Incorrect number of operands provided." + "\n"
                #quit()
            if instruct_root_p[1].upper() in ["FLAGS", "R0", "R1", "R2", "R3", "R4", "R5", "R6"] or instruct_root_p[1].lower() in operations.keys():
                errors += "ERROR @Line "+str(linec) + ": Illegal use of reserved key words(register name, operation name)." + "\n"
                # quit()
                #raise Exception("Illegal use of reserved key words(register name, operation name).")
            if instruct_root_p[1].lower() in labels.keys():
                errors += "ERROR @Line "+str(linec) + ": Label with given variable name already exists." + "\n"
                #quit()
                #raise Exception("Label with given variable name already exists.")
            if instruct_root_p[1].lower() in vars.keys():
                errors += "ERROR @Line "+str(linec) + ": Variable with given variable name already exists." + "\n"
                #quit()
        ...
        if instruct_root_p[0][-1] == ":":
            ...
            if instruct_root_p[0][:-1:].lower() in vars.keys():
                errors += "ERROR @Line "+str(linec) + ": Variable with given label name already exists." + "\n"
               #quit()
                #raise Exception("Variable with given label name already exists.")
            if len(instruct_root_p) == 1:  # ask doubt for empty labels
                errors += "ERROR @Line "+str(linec) + ": Label not defined(Empty Label given)" + "\n"
                #quit()
            for member in instruct_root_p[0][:-1:].lower():
                if not(member.isalnum()) and not(member == '_'):
                    errors +=  "ERROR @Line "+str(linec) + ": Label name can only contain alphanumeric or '_' characters" + "\n"
                    # quit()
            if instruct_root_p[0][:-1:].lower() in labels.keys():
                errors += ("ERROR @Line "+str(linec) +
                      ": Label with given lable name already exists.") + "\n"
               # quit()
            else:
    ...
    if(hlt_c > 1):
        errors += ("ERROR: Multiple hlt statements declared") + "\n"
       #quit()

    # if instruct_root_p[0] != "hlt": #changed
    if ((instruct_root_p[0].lower() != "hlt" and len(instruct_root_p) == 1) or (instruct_root_p[0][-1] == ':' and len(instruct_root_p) == 2 and instruct_root_p[1].lower() != 'hlt')):
        errors += ("ERROR: No exit statement(hlt) in code.") + "\n"
        #quit()
    if len(instruct_root_p) > 2:
        errors += ("ERROR @Line "+str(linec)+": Incorrect/Absent hlt declaration") + "\n"
       # quit()
    ...
    for instruction in code.splitlines():  # for instruction in code.readlines():  #changed
        ...
        if instruct_root[0].lower() not in operations or instruct_root[0].lower() == "mov_im" or instruct_root[0].lower() == "mov_f":
            errors += ("ERROR @Line "+str(linec)+": " +
                  instruct_root[0] + " is not a valid command") + "\n"
           # quit()
        ...
        if instruct_root[0].lower() == "mov" and len(instruct_root) == 3 and instruct_root[2][0] == "$":
            if(len(instruct_root[2]) == 1):
                errors += ("ERROR @Line "+str(linec) +
                      ": Immediate Value not specified") + "\n"
               # quit()
            try:
                int(instruct_root[2][1:])
                if(int(instruct_root[2][1:]) != float(instruct_root[2][1:])):
                    errors += ("ERROR @Line "+str(linec) +
                          ": Immediate Value can't be a floating point number") + "\n"
                  #  quit()
            except:
                errors += ("ERROR @Line "+str(linec) +
                      ": Immediate Value given is not valid ") + "\n"
                #quit()
        ...
        if(instruct_root[0].lower() not in operations.keys()):
            errors += ("ERROR @Line "+str(linec)+": Illegal operation name used ") + "\n"
            #quit()
        ...
        if interpreted[1] == 'A':
            if len(instruct_root) != 4:
                errors += ("ERROR @Line "+str(linec) +
                      ": Incorrect number of operands provided.") + "\n"
               #quit()
            ...
            if (instruct_root[1].upper() not in r_names or instruct_root[2].upper() not in r_names or instruct_root[3].upper() not in r_names):
                errors +=("ERROR @Line "+str(linec)+": Illegal register name used") + "\n"
               # quit()
            ...
            if (int(instruct_root[2][1::]) > 255 or int(instruct_root[2][1::]) < 0):
                errors += ("ERROR @Line "+str(linec)+": Illegal Immediate values") + "\n"
                #quit()
            ...
            if (instruct_root[2].upper() in r_names):
                errors += ("ERROR @Line "+str(linec)+": Illegal use of register " +
                      instruct_root[2]+", memory address required") + "\n"
                #quit()
            if instruct_root[2].lower() not in vars:
                errors +=("Definition Error @Line "+str(linec) +
                      ":  Variable '" + instruct_root[2] + "' not Defined") + "\n"
               #quit()
    ...
    #global linec
    if quiting == 0:
        errors += ("ERROR @Line "+str(linec)+": General Synatax Error") + "\n"
    print(errors)
